...
class DYNMPCControllerNode(Node):
	def __init__(self):
		...
		self.model 	  = dynamics.Dynamic()
		time_step     = 0.04
		
		# centerline
		# self.Horizon  = 12
		# COST_Q        = np.diag([0.12, 0.12])
		# COST_P        = np.diag([0.12, 0.])
		# COST_R        = np.diag([0.55, 1.85])
		# COST_S        = np.diag([0.12, 0.])

		# raceline
		self.Horizon  = 11
		# COST_Q        = np.diag([0.08, 0.08])
		# COST_P        = np.diag([0.1, 0.])
		# COST_R        = np.diag([0.5, 1.55])
		# COST_S        = np.diag([0.35, 0.])
		COST_Q        = np.diag([0.12, 0.12])
		COST_P        = np.diag([0.05, 0.])
		COST_R        = np.diag([0.65, 2.65])
		COST_S        = np.diag([0.05, 0.])
		track_width   = 2.4
		...
		n_steps       = 4000 # ~ 1 laptime, timestep:0.02 sec
	...
	def pose_callback(self, msg:Odometry):
		...
		self.inputs[1, self.step] = self.states[2, self.step] - self.states[2, self.step-1] # Steering vel
		# Update ω and β with numerical integration (Heun's method)
		end         = tm.time()
		h           = end-start
		self.time  += h
		ω_β_next    = self.model.odeintRK6(x0, h, self.inputs[:, self.step]) 
		...
		self.vel_pub.publish(self.vel_cmd)
		# print("X: {:.4f}, Y: {:.4f}, Velocity: {:.4f}, Acceleration: {:.4f}, Steering: {:.5f}".format(self.states[0, self.step], self.states[1, self.step], velocity, acc, steering_ang))
		
		# Record states
		check_point = 1.5 # 95~4 seconds
