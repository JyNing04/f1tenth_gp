...
JOYSTICK_CODE_VALUE_MAP = {
    ...
    "GC201 Controller1.00": (GC201_CODE_MAP, GC201_VALUE_MAP)
...
class JoystickRos2(Node):
    ...
    def __init__(self):
        ...
        # Joy publisher
        qos_profile_real_points = rclpy.qos.qos_profile_system_default
        self.publisher_ = self.create_publisher(Joy, 'joy', qos_profile_real_points)
    ...
    def normalize_key_value(self, key_value_min, key_value_max, key_value):
        ...
        if (abs(normalized) > self.deadzone):
            ...
            return 0.0
    def set_joy(self, axes, buttons):
        self.joy.axes = [0.0 for i in range(axes)]
        self.joy.buttons = [0 for i in range(buttons)]
    def run(self):
        ...
        while rclpy.ok():
            ...
            axes_num = 6 if gamepad.name == "GC201 Controller1.00" else 8
            button_num = 13 if gamepad.name == "GC201 Controller1.00" else 11
            self.set_joy(axes_num, button_num)
            # print(len(self.joy.buttons))
            while True:
                try:
